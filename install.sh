...
if [[ "$symlink_answer" == "y" ]]; then
  rm -rf "${HOME}/.i3"
  for fl in vim vimrc screenrc tmux.conf bashrc gitconfig i3status.conf i3 urxvt Xresources
  do
...
if [[ "$install_answer" == "y" ]]; then
  ...
  sudo apt-get install curl rxvt-unicode-256color fonts-inconsolata \ 
    xclip vim git python-pip python-setuptools libssl-dev build-essential
